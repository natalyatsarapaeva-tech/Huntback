// Конфиг Huntback (ТЗ §10.1 в редакции 1.2): модели, пороги, стоп-листы и
...
// ── Собранный конфиг ────────────────────────────────────────────────────────
export interface HuntbackConfig {
  models: ModelsConfig;
...
export const DEFAULT_CONFIG: HuntbackConfig = {
 ...
 */
export function resolveConfig(overrides?: DeepPartial<HuntbackConfig> | null): HuntbackConfig {
...
export function degradeModel(cfg: HuntbackConfig, model: string): string | null {
  return cfg.models.fallback[model] ?? null;
}

/** Модель, назначенная операции (§10.1). */
export function modelFor(cfg: HuntbackConfig, op: Operation): string {
